...
export class LocaisComponent {
  ...
  async dadosGrid ( ) {
    ...
    let request = await fetch(`http://localhost:8000/grid/locais`, {
      method: "POST",
  ...
  subIncluir ( ) {
    ...
    (document.querySelector('.sub-dados') as HTMLElement).removeAttribute('hidden');
    (document.querySelector('.sub-grid') as HTMLElement).setAttribute('hidden','');
